// Verifies /members/terms renders the PUBLISHED agreement, in a real browser.
//
// Uses a THROWAWAY MEMBER (member_no ZZ-TEST9), never a real one: the page needs
...
const PASS='ZZ-Probe-Pass-2691', MNO='ZZ-TEST9', EMAIL='zz-terms@example.invalid'
...
try {
  ...
  // The PRIVACY NOTICE gate stands in front of every /members page: a member with
  // a member_no and no privacy consent is redirected to /members/agree, so
  // /members/terms is unreachable until that is answered. Satisfy it for the
  // throwaway the same way the portal would, then carry on.
  const { createHmac } = await import('node:crypto')
  const b64u = b => Buffer.from(b).toString('base64url')
  const now = Math.floor(Date.now()/1e3)
  const H = b64u(JSON.stringify({alg:'HS256',typ:'JWT'}))
  const P = b64u(JSON.stringify({sub:uid,aud:'authenticated',role:'authenticated',iat:now,exp:now+900}))
  const jwt = `${H}.${P}.${b64u(createHmac('sha256', env.SUPABASE_JWT_SECRET).update(`${H}.${P}`).digest())}`
  const asMember = { apikey: env.NEXT_PUBLIC_SUPABASE_ANON_KEY, Authorization: `Bearer ${jwt}`, 'Content-Type':'application/json' }
  const consent = await fetch(`${U}/rest/v1/rpc/record_my_consent`, { method:'POST', headers:asMember,
    body: JSON.stringify({ p_doc_key:'privacy', p_granted:true, p_user_agent:'verification', p_evidence:{ probe:true } }) })
  ok(consent.ok, 'privacy consent recorded for the throwaway (clears the gate)', `HTTP ${consent.status}`)
