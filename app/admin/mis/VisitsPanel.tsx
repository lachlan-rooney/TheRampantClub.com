...
export default function VisitsPanel({ memberNo, onAfterChange }: { memberNo: string; onAfterChange?: () => void }) {
  ...
  return (
    <div style={panel}>
      ...
      <ConfirmModal
        open={!!confirmArchive}
        eyebrow="⚠ ARCHIVE VISIT"
        title="Archive this visit?"
        subject={confirmArchive ? `${fmtDate(confirmArchive.visit_date)}${confirmArchive.space ? ` · ${confirmArchive.space}` : ''}` : undefined}
        body="Hides the visit from the live log and stops it counting toward M (visit cadence). The row is preserved and can be restored from “Show archived”."
        confirmLabel="Archive visit"
        busyLabel="Archiving…"
        busy={archiveBusy}
