  ...
  return (
    <>
      ...
      {noGrid ? (
        ...
        <div style={{
          ...wallGrid,
          gridTemplateColumns: `36px repeat(${gridDims.cols}, minmax(72px, 1fr))`,
        }}>
          {/* column headers */}
          <div />
          {Array.from({ length: gridDims.cols }, (_, c) => (
...
function RowFragment({ rowIdx, cols, lockerByPos, onOpen, filter }: {
  rowIdx: number
  cols: number
  ...
  return (
    <>
      ...
      {Array.from({ length: cols }, (_, c) => {
        const l = lockerByPos.get(`${rowIdx}-${c + 1}`)
        if (!l) return <div key={`empty-${c}`} style={tileGhost} />
...
// ── DRAWER ──────────────────────────────────────────────────────────
function LockerDrawer({ locker_no, members, onClose, onChange }: {
  locker_no: string
  members: MemberLite[]
  onClose: () => void
  onChange: () => void
  ...
  const [newBottle, setNewBottle] = useState({ bottle_name: '', distillery: '', age: '', abv: '', fill_pct: '100' })
  ...
  const addBottle = async () => {
    if (!newBottle.bottle_name.trim()) return
    setAdding(true)
    await fetch(`/api/admin/lockers/${locker_no}/contents`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        bottle_name: newBottle.bottle_name,
        distillery: newBottle.distillery || null,
        age: newBottle.age || null,
        abv: newBottle.abv || null,
        fill_pct: Number(newBottle.fill_pct) || 100,
      }),
    })
    setNewBottle({ bottle_name: '', distillery: '', age: '', abv: '', fill_pct: '100' })
    setAdding(false)
    load()
    onChange()
  }
          ...
          <>
            ...
            <Section title={`Contents · ${contents.length} ${contents.length === 1 ? 'bottle' : 'bottles'}`}>
              ...
              <div style={addBottleBlock}>
                <div style={editLabel}>Add bottle</div>
                <input
                  value={newBottle.bottle_name}
                  onChange={e => setNewBottle({ ...newBottle, bottle_name: e.target.value })}
                  placeholder="Bottle name (e.g. Hibiki Harmony)"
                  style={inputStyle}
                />
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <input value={newBottle.distillery} onChange={e => setNewBottle({ ...newBottle, distillery: e.target.value })} placeholder="Distillery" style={{ ...inputStyle, flex: 2, minWidth: 120 }} />
                  <input value={newBottle.age} onChange={e => setNewBottle({ ...newBottle, age: e.target.value })} placeholder="Age" type="number" style={{ ...inputStyle, flex: 1, minWidth: 70 }} />
                  <input value={newBottle.abv} onChange={e => setNewBottle({ ...newBottle, abv: e.target.value })} placeholder="ABV %" type="number" step="0.1" style={{ ...inputStyle, flex: 1, minWidth: 70 }} />
                  <input value={newBottle.fill_pct} onChange={e => setNewBottle({ ...newBottle, fill_pct: e.target.value })} placeholder="Fill %" type="number" style={{ ...inputStyle, flex: 1, minWidth: 70 }} />
                </div>
                <button onClick={addBottle} disabled={!newBottle.bottle_name.trim() || adding} style={{ ...btnPrimary, marginTop: 6, opacity: !newBottle.bottle_name.trim() ? 0.4 : 1 }}>
