...
export default function AdminReports() {
  ...
  const generate = async () => {
    ...
    else setMsg(d.error || 'Failed to generate')
  }
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontFamily: SERIF, fontSize: 24, fontWeight: 500, color: '#E5D4C2', letterSpacing: '0.04em', marginBottom: 8 }}>Weekly Report</h1>
          <p style={{ fontFamily: MONO, fontSize: 11, color: '#B2AA98', marginBottom: 20, lineHeight: 1.6, maxWidth: 620 }}>
            Auto-drafted each Monday with the week&rsquo;s data. Add the narrative, submit for approval, then it&rsquo;s sent to Shawn (17:00 VN Mon) with a link to the full report.
          </p>
        </div>
        <button onClick={generate} disabled={busy} style={{ ...btnPrimary, opacity: busy ? 0.5 : 1 }}>
          {busy ? 'Generating…' : 'Generate this week&rsquo;s draft'}
        </button>
      </div>
      {msg && <div style={{ fontFamily: MONO, fontSize: 11, color: '#C27070', marginBottom: 12 }}>{msg}</div>}

      <div style={card}>
        {loading ? <div style={{ fontFamily: MONO, fontSize: 11, color: '#7E7864' }}>Loading…</div> : rows.length === 0 ? (
          <div style={{ fontFamily: MONO, fontSize: 12, color: '#B2AA98' }}>No reports yet — generate this week&rsquo;s draft to begin.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>{['Week', 'Headline', 'Status', 'Views', ''].map(h => <th key={h} style={th}>{h}</th>)}</tr></thead>
            <tbody>
              {rows.map(r => {
                const s = STATUS[r.status] || STATUS.draft
                return (
                  <tr key={r.id} style={{ borderTop: '1px solid rgba(229,212,194,0.06)' }}>
                    <td style={td}>{fmt(r.period_start)} – {fmt(r.period_end)}{r.include_financials ? <span style={finTag}>+ financials</span> : ''}</td>
                    ...
                    <td style={{ ...td, textAlign: 'right' }}><Link href={`/admin/reports/${r.id}`} style={btnGhost}>Open</Link></td>
                  </tr>
                )
              })}
            </tbody>
